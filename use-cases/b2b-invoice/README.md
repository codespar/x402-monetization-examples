...
- This link has no Pix rail, so this example does not move, or claim to move, any BRL. Pix on payment links generally is code-complete but still needs Celcoin production credentials before it moves real BRL; see [payment-link](../../payment-link) and the top-level [README](../../README.md#honesty).
